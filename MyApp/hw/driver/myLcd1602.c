...
static bool     lcd_ok = false; // 초기화 성공 여부: false이면 모든 호출 무시
...
void lcd1602Backlight(bool on)
{
  if (!lcd_ok) return;
  backlight_state = on;
  uint8_t val = backlight_state ? 0x08 : 0x00;
  HAL_I2C_Master_Transmit(&hi2c1, lcd1602_addr, &val, 1, 10);
}
